...
void main(){
  ...
  float elevation = sin(modelPosition.x * uFrequency.x + uTime * uSpeed  )
                    * sin(modelPosition.y * uFrequency.x + uTime * uSpeed ) 
                    * uElevation;

        for(float i = 1.0; i <= uSmallWaveIterations; i++){

        elevation -= abs(cnoise(vec3(modelPosition.xy * uSmallWaveFrequency * i, uTime * uSmallWaveSpeed ))* uSmallWaveElevation / i);
        }
